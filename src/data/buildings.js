...
export function getBuildingDef(buildingOrType) {
  ...
  if (typeof buildingOrType === "string") return BUILDING_DEFS[buildingOrType] ?? null;
  if (buildingOrType.type && BUILDING_DEFS[buildingOrType.type]) return BUILDING_DEFS[buildingOrType.type];
  if (buildingOrType.key && BUILDING_DEFS[buildingOrType.key]) return BUILDING_DEFS[buildingOrType.key];
  return null;
}
